...
-- 英雄状态
BattleBuffer = xclass('BattleBuffer')
...
function BattleBuffer:Serialize( ... )
	...
	if self.haloCounter then
		...
		tBuffer.Halo.Interval = self.haloInterval
	end
...
function BattleBuffer:DeSerialize( _tBuffer, _battleUnit, ... )
	...
	if _tBuffer.Halo then
		...
		self.haloInterval = _tBuffer.Halo.Interval
	...
	gBattleManager:RegisterSnapShotPushEndEvent(function( ... )
		local owner = gBattleRecord:GetBattleUnit(_tBuffer.OwnerId)
		if owner then
			self.owner = owner
		else
...
function BattleBuffer:Destroy( ... )  
	-- gBattleManager:AddBattleLog(string.format('RemoveBuffer Time[%d] Buffer[%d] Unit[%d] Owner[%d]', gBattleTime, self.bufferRes.id, self.battleUnit.unitId, self.owner.player.unitId))
...
function BattleBuffer:Init( _bufferResId, _battleUnit, _owner, ... )  
	...
	-- gBattleManager:AddBattleLog(string.format('AddBuffer Time[%d] Buffer[%d] Unit[%d] Owner[%d]', gBattleTime, _bufferResId, _battleUnit.unitId, _owner.unitId))
...
function BattleBuffer:Update( _deltaTime, ... )  
	if self.unitType ~= BattleUnitType.GRID then
		...
		local battleUnit = self.battleUnit
		if haloCounter and battleUnit:IsHaloValid() then
			...
			if haloCounter >= haloInterval then
				...
				if talentRes then
					for i = 1, #talentRes.actionList do
						...
						local triggerParam = BattleTriggerParam_New(battleUnit, nil, nil, nil)
						gBattleTrigger:FireAction(actionId, battleUnit, self.owner, triggerParam)
						BattleTriggerParam_Destroy(triggerParam)
					end
				end
				...
				self.haloInterval = BattleFormula.GetValue(self.bufferRes.haloIntervalList[haloIntervalIndex], self, battleUnit)
...
function BattleBuffer:AddBufferLayer( _owner, _useBetterLayer, _isInit, ... )  
	...
	local layer = BattleBufferLayer(self, gBattleTime, _owner, nil)

	-- 解析状态数据
	local bufferValueList, isLayerBindGrade = self:ParseBufferValue(layer)
	...
	-- 插入状态层
	layer.bindGrade = isLayerBindGrade
...
-- 移除顶层叠加
function BattleBuffer:RemoveBufferLayer( _layerIndex, _owner, _keepInList, _checkRemove )  
	local layerIndex = _layerIndex or 1
	local layer = false
	local layerList = self.layerList
	if layerIndex ~= 0 then
		layer = layerList[layerIndex]
	elseif _owner and _owner.unitId then
		for i = 1, #layerList do
			local bufferLayer = layerList[i]
			if bufferLayer.owner.unitId == _owner.unitId then
				layerIndex = i
				layer = bufferLayer
				break
			end
	...
	if _checkRemove and #layerList == 0 then
		self.battleUnit:RemoveBuffer(self)
	end
end
...
function BattleBuffer:RefreshBufferLayer( _layer )
	...
	local bufferValueList = self:ParseBufferValue(_layer)
...
-- 解析状态数据
function BattleBuffer:ParseBufferValue( _layer, ... )  
	...
	for i = 1, valueNum do
		...
		local value, bindGrade = BattleFormula.GetValue(math.abs(formulaId), _layer, self.battleUnit)
		if formulaId < 0 then
			value = value * -1
		end
...
function BattleBuffer:GetGrade( ... )
	...
	if player then
		return player:GetTowerGradeByBaseId(self.bufferRes.towerId)
	end
	return 0
end
...
function BattleBuffer:GetLevel( ... )
	local player = self.owner.player
	if player then
		return player:GetTowerLevelByBaseId(self.bufferRes.towerId)
	end
	return 0
end